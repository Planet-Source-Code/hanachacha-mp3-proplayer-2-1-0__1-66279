...
'Modify by: Dante  > dantex_765@hotmail.com
'Modify by:Hanachacha _ add new property <HightlightBorderColor>
'                     _ <HightLightPicture>
'                     _ <Bar color>
Option Explicit
Public Type MenuSetting
    ...
    HightLightPicture                         As StdPicture
    Icon                                      As Collection
    'Font                                      As StdFont
    FontName                                  As String
    FontBold                                  As Boolean
...
Public Type ColorSetting
    ...
    HilightBorderColor                        As OLE_COLOR
    ...
    If aDIS.ItemData = 2 Then
        ...
        With lastDISRect
            If (.Top < aDIS.rcItem.Top And aDIS.itemID > gStartTopID And .Right <> 0 And aDIS.itemID = lastDISID + 1) Then
                ...
                boxRect.Right = aRect.Right - aRect.Left - GetSystemMetrics(SM_CXBORDER) * 2
...
'-------------------------------------------------------------------------
    If (aDIS.ItemData = 2 And aDIS.itemID = gLastTopMenuID) Then
        ...
        With boxRect
            ...
            .Right = aRect.Right - aRect.Left - GetSystemMetrics(SM_CXBORDER) * 2
            .Top = aDIS.rcItem.Top
    ...
    If isSep Then
        With boxRect
            ...
            .Right = backBuffRECT.Right - 2
            .Bottom = .Top + 1 'backBuffRECT.Bottom - 2
        End With
        
        abrush = CreateSolidBrush(CustomColor.BarColor)
    ...
    If (aDIS.itemState And ODS_CHECKED) = ODS_CHECKED Then
        ...
        If lPic Is Nothing Or lMask Is Nothing Then
    ...
    If (aDIS.itemState And ODS_CHECKED) = ODS_CHECKED Then
        ...
        DeleteObject colBmp
        '==============================================
        ' 3D BORDER 'set up a rectangle to draw the upraised edge for checked
        '==============================================
        With boxRect
            .Top = 0
            .Left = CustomMenu.PosX + 1
            .Right = .Left + mnuItemWid + 1
    ...
    '==============================================
    ' 3D BORDER 'set up a rectangle to draw the upraised edge for selected
    '==============================================
    With boxRect
        .Top = 0
        .Left = CustomMenu.PosX + 1
...
drawMnuBitmap_picsDone:
    ...
    ' Draw the edge
        If CustomMenu.HightLightPicture Is Nothing Then
        '=================================================
        ' HILIGHT BORDER COLOR 'set up a rectangle to draw the "highborderlight" color
        '=================================================
            With boxRect
                .Left = IIf(noPics, backBuffRECT.Left + CustomMenu.PosX, .Right) + 1
                .Top = backBuffRECT.Top
                .Bottom = backBuffRECT.Bottom
                .Right = backBuffRECT.Right
            End With
            ' create a brush in the highlight color
            abrush = CreateSolidBrush(CustomColor.HilightBorderColor)
        ...
        '=================================================
            With boxRect
                .Left = IIf(noPics, backBuffRECT.Left + CustomMenu.PosX, .Left - 1) + 2
                .Top = backBuffRECT.Top + 1
                .Bottom = backBuffRECT.Bottom - 1
                .Right = backBuffRECT.Right - 1
        ' create a brush in the highlight color
            End With
        ...
        Else
            ' create a brush
            abrush = CreatePatternBrush(CustomMenu.HightLightPicture.handle)
            With boxRect
                .Left = IIf(noPics, backBuffRECT.Left + CustomMenu.PosX, .Right) + 1
                .Top = backBuffRECT.Top
                .Bottom = backBuffRECT.Bottom
    ...
    If lPic.handle <> 0 Then
        ...
        Else
            ...
            ' Blt the picture
            'BitBlt backBuffDC, CustomMenu.PosX + 2, backBuffRECT.Top + (backBuffRECT.Bottom - backBuffRECT.Top - aBmp.bmHeight) / 2, aBmp.bmWidth, aBmp.bmHeight, picDC, 0, 0, vbMergeCopy
            TransparentBlt backBuffDC, CustomMenu.PosX + 2, backBuffRECT.Top + (backBuffRECT.Bottom - backBuffRECT.Top - aBmp.bmHeight) / 2, aBmp.bmWidth, aBmp.bmHeight, picDC, 0, 0, aBmp.bmWidth, aBmp.bmHeight, &HFFFFFF
        End If
